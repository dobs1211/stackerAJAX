//usually, the document ready function should be at the bottom of the app,
//but in this particular case, the app logic needs to take the input from the user first, so document ready is first
$(document).ready(function () {
    //Step 1 - get the input from the user regarding the unaswered questions search
    $('.unanswered-getter').submit(function (event) {
        //if page refreshes when you submit the form use "preventDefault()" to force JavaScript to handle the form submission
...
var getUnanswered = function (tags) {
    ...
    $.ajax({
        ...
        .done(function (result) { //this waits for the ajax to return with a successful promise object
            //show the number of results for an API call
            var searchResults = showResultsCounter(request.tagged, result.items.length);
            $('.search-results').html(searchResults);
            //loop through all the results and pick each of them to display in its own container
            $('.search-results').html(searchResults);

            //loop throu all the results and pick each of them to display in its own container
            //$.each is a higher order function. It takes an array and a function as an argument.
            //the fucntion is executed once for each item in the array.
            $.each(result.items, function (i, item) {
                //call the following function (showQuestion(item))
...
var getInspiration = function (tag) {
    ...
    var result = $.ajax({
        ...
        .done(function (result) {
            ...
            //loop thru all the results and pick each of them to display in its own container
            //$.each is a higher order function. It takes an array and a function as an argument.
...
var showQuestion = function (question) {
    ...
    //set the "asked date" property in result
...
//Step 3 - (getInspiration - top users search) - using the JSON response, populate the relevant part of your HTML with the variable
...
//Extra Function
//this function takes the results objects from StackOverflow and returns the number of results and tags to be appended to DOM
